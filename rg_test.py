...
class NeuralNetwork:
    def __init__(self, layer_sizes, learning_rate=0.01, epochs=10000, eps=0.001, beta=0.9, datatype='1'):
        ...
        self.error = []  # List to store MSE values for each epoch
    ...
    def train(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            self.error.append(loss)  # Collect MSE for each epoch
    ...
    def plot_training_results(self, all_errors):
        # Calculate average MSE for each epoch across all folds
        max_epochs = max(len(errors) for errors in all_errors)
        avg_errors = np.zeros(max_epochs)
        
        for epoch in range(max_epochs):
            epoch_errors = [errors[epoch] for errors in all_errors if epoch < len(errors)]
            avg_errors[epoch] = np.mean(epoch_errors)
        
        plt.figure()
        plt.plot(avg_errors, label='Average MSE')
        plt.title('Average MSE vs. Epoch for All Folds')
        plt.xlabel('Epoch')
...
if __name__ == '__main__':
    data_type = '1'  # Change this to '1' for regression, '2' for classification
